...
Debugger.prototype.scriptFromUri = function(uri) {
    var scriptId = this.scriptUriToIdMap[uri];
    if (!scriptId) {
        return null;
    }
...
Debugger.prototype.newScript = function(script) {
    ...
    var scriptId = s ? s.scriptId : script.scriptId; //if the script has already been added through 'enumerateScripts()', use the id generated there
    script.scriptId = scriptId; // overwrite the scriptId, so DevTools doesn't get confused
