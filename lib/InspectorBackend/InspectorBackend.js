...
InspectorBackend.prototype.Debugger_pause = function(params, callback) {
    var threadId = _.first(_.keys(this.dbg.threads));
    this.dbg.suspend(threadId, function() {
        callback({
            result: {}
        });
    });
...
InspectorBackend.prototype.dispatchMessage = function(method, params) {
    ...
    if (this.messageLoggingEnabled) {
        logger.info("dispatching message: " + JSON.stringify(msg));
    }
...
InspectorBackend.prototype.Runtime_getProperties = function(params, callback) {
    ...
    if (params.accessorPropertiesOnly) {
        //It looks like Ti debugger doesn't enumerate these, so we skip them
        callback({result: {}});
        return;
    }
...
InspectorBackend.prototype.remoteObjectFromVar = function(v, frame, parentName) {
    ...
    if (this.varIsObject(v)) {
        ...
        if (type == 'object' && value == '[object Function]') {
            type = 'function';
        }
        _.extend(remoteObj, {
            type: type,
